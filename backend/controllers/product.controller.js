...
export const getAllProducts = async (req, res) => {
  try {
    ...
    console.log("Error in getAllProducts endpoint " + error.message);
...
export const getFeaturedProducts = async (req, res) => {
  try {
    ...
    console.log("Error in getFeaturedProducts endpoint " + error.message);
    return res
      .status(500)
...
export const deleteProduct = async (req, res) => {
  try {
    ...
    if (product.image) {
      const publicId = product.image.split("/").pop().split(".")[0]; // this will get id of image
      try {
...
export const getRecommendatedProduct = async (req, res) => {
  try {
...
export const toggleFeaturedProduct = async (req, res) => {
  try {
    ...
    console.log("Error in toggleFeaturedProduct endpoint " + error.message);
    res.status(500).json({ message: "Server Error", error: error.message });
  }
};
