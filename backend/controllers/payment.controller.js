...
export const createCheckoutSession = async (req, res) => {
  try {
    ...
    console.log("Error in payment endpoint", error.message);
  ...
  try {
    ...
    if (session.payment_status === "paid") {
      ...
      res.status(200).json({
        success: true,
        message:
          "Payment sucessfull, order created, and coupon destivated if use.",
        orderId: newOrder._id,
      });
    }
  } catch (error) {
    console.log("Error in checkoutSuccess endpoint", error.message);
    res.status(500).json({ message: "Server Error", error: error.message });
  }
};
